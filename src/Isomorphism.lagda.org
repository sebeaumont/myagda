...
This seems a strange defintion; I mean even over the Nats how can we be
sure that two functions produce the same output (apart from running them
over all possible inputs from the domain){!!} Doesn't seem very
constructive to me. I wonder what Brouwer would say?
