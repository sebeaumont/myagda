...
[[https://github.com/plfa/plfa.github.io/issues/428][GitHub issue 428]] and this note: [[id:F449FE74-85B1-4C6B-B188-814649D48CD1][View From The Left]].
#+end_comment 
...
#+begin_src agda2
⊎-distrib-× : ∀ {A B C : Set} → (A × B) ⊎ C ≲ (A ⊎ C) × (B ⊎ C)
⊎-distrib-× ._≲_.to (inj₁ x) = ⟨ inj₁ (x .proj₁) , inj₁ (x .proj₂) ⟩
⊎-distrib-× ._≲_.to (inj₂ x) = ⟨ inj₂ x , inj₂ x ⟩
⊎-distrib-× ._≲_.from ⟨ inj₁ x , inj₁ y ⟩ = inj₁ ⟨ x , y ⟩
⊎-distrib-× ._≲_.from ⟨ inj₁ x , inj₂ y ⟩ = inj₂ y
⊎-distrib-× ._≲_.from ⟨ inj₂ x , inj₁ y ⟩ = inj₂ x
⊎-distrib-× ._≲_.from ⟨ inj₂ x , inj₂ y ⟩ = inj₂ x
⊎-distrib-× ._≲_.from∘to (inj₁ ⟨ x , y ⟩) = refl
⊎-distrib-× ._≲_.from∘to (inj₂ z) = refl
#+end_src
