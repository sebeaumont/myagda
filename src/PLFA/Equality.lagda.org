...
* Propositional or Typeal Equality

Unfortunately terminology and notation has differed quite confusingly[fn:2] in
the type theory community considered over time; many authors use ≡ for
/definitional equality/ which is hardwired in most programming languages as
= here as in other places we will let = stand for definitional or
/judgemental equality/ which can be interpreted as syntactic. We will use ≡
for /propositional/ or /typal equality/.[fn:1] 
...
embarking on that journey. It's worth noting that definition of
propositional equality there is a little different.
...
* Footnotes
[fn:2] This is doubly unfortunate as drawing correct distinctions between
these often reasonably misunderstood notions due to our prior exposure to
equality as math students is central to an understanding of this subject.

[fn:1] Propostional equality requires us to provide a proof or construct a
witness of the equality which can only be constructed in the TT expressed
here using the /refl/ introduction or constructor. Later we will see how
other notions of equality and equivalence may be formed.
