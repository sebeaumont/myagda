#+setupfile: src/org-source-setup.org
...
And some ideas and inspiration (as well as convincing me, proof by
contradiction) that unicode ligatures and symbols are good for
readability iff used with sparsity and taste.
...
Another good read is [[https://leanpub.com/certainty-by-construction/][Certainty by Construction]] which is an altogether
...
and discovered a few Agda hints, tip and techniques.
...
** DONE Be good if the whole thing compiled into a library.
...
** TODO Create toplevel package
so we can avoid ambiguity if we import this stuff as a library.
** TODO Maybe remove dogfood references in every header.
